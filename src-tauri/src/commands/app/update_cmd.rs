...
const GITHUB_RELEASE_API_BASE: &str = "https://api.github.com/repos/lookfeeb/KiroHub/releases/tags";
#[cfg(target_os = "linux")]
...
fn normalize_release_tag(version: &str) -> Result<String, String> {
...
#[tauri::command]
pub async fn get_release_changelog(version: String) -> Result<String, String> {
    let tag = normalize_release_tag(&version)?;
    let client = build_http_client()?;
    let url = format!("{GITHUB_RELEASE_API_BASE}/{tag}");

    let response = client
        .get(&url)
        .header(reqwest::header::USER_AGENT, "KiroHub")
        .send()
        .await
        .map_err(|e| format!("请求更新内容失败: {e}"))?;
    ...
    let release: GithubRelease = response
        .json()
        .await
        .map_err(|e| format!("解析更新内容失败: {e}"))?;

    Ok(release
        .body
        .map(|body| body.trim().to_string())
        .filter(|body| !body.is_empty())
        .unwrap_or_else(|| "暂无更新说明".to_string()))
}
...
mod tests {
    use super::{
        ...
        normalize_release_tag, parse_version_parts,
    };
    ...
    #[test]
    fn normalize_release_tag_accepts_plain_or_prefixed_versions() {
        assert_eq!(normalize_release_tag("0.0.5"), Ok("v0.0.5".to_string()));
        assert_eq!(normalize_release_tag("v0.0.5"), Ok("v0.0.5".to_string()));
        assert!(normalize_release_tag("").is_err());
        assert!(normalize_release_tag("0.0.5/evil").is_err());
    }
}
