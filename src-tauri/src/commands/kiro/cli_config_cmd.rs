...
fn write_pretty_json(path: &Path, value: &Value) -> Result<(), String> {
    ...
    atomic_write(path, &content, &path.display().to_string())
}
...
pub async fn write_claude_code_config(base_url: String, api_key: String) -> Result<String, String> {
    ...
    let config_obj = config
        .as_object_mut()
        .ok_or_else(|| "Claude 配置根节点不是对象".to_string())?;
    let env = config_obj
        .entry("env")
    ...
    let env_obj = env
        .as_object_mut()
        .ok_or_else(|| "Claude 配置 env 节点不是对象".to_string())?;
...
pub async fn write_codex_cli_config(
    ...
    atomic_write(&config_path, &doc.to_string(), "config.toml")?;
