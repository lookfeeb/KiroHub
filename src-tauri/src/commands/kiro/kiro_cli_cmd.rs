...
use crate::commands::common::{extract_user_info, get_usage_by_provider, refresh_token_by_provider};
...
const CLI_TOKEN_AUTO_REFRESH_BEFORE_SECONDS: i64 = 180;
...
fn cli_token_expires_within(expires_at: Option<&str>, seconds: i64) -> bool {
    let Some(expires_at) = expires_at else {
        return false;
    };
    chrono::DateTime::parse_from_rfc3339(expires_at)
        .map(|expires| chrono::Utc::now() + chrono::Duration::seconds(seconds) >= expires)
        .unwrap_or(true)
}

fn cli_snapshot_needs_refresh(snapshot: &crate::kiro::cli::KiroCliDbSnapshot) -> bool {
    snapshot
        .token_entries
        .first()
        .and_then(|entry| entry.parsed_token.as_ref())
        .is_some_and(|token| {
            !token.refresh_token.is_empty()
                && cli_token_expires_within(
                    token.expires_at.as_deref(),
                    CLI_TOKEN_AUTO_REFRESH_BEFORE_SECONDS,
                )
        })
}
...
/// 读取 CLI 数据库快照（前端展示用）
#[tauri::command]
pub async fn read_cli_db_snapshot(
    db_path: String,
) -> Result<crate::kiro::cli::KiroCliDbSnapshot, String> {
    let expanded_path = expand_home_dir(&db_path)?;
    read_cli_db_snapshot_with_auto_refresh(&expanded_path).await
}

async fn refresh_cli_db_token_inner(
    expanded_path: &str,
) -> Result<crate::kiro::cli::KiroCliDbSnapshot, String> {
    let snapshot = crate::kiro::cli::read_cli_db_snapshot(expanded_path)?;
    let entry = snapshot
        .token_entries
        .first()
        .ok_or_else(|| "kiro-cli 数据库中没有 token".to_string())?;
    let token = entry
        .parsed_token
        .as_ref()
        .ok_or_else(|| "kiro-cli token 无法解析".to_string())?;

    let raw: serde_json::Value = serde_json::from_str(&entry.value_json)
        .map_err(|e| format!("kiro-cli token JSON 解析失败: {e}"))?;
    let is_social = entry.key.contains("social");
    let provider = if is_social {
        match raw.get("provider").and_then(|v| v.as_str()).unwrap_or_default().to_ascii_lowercase().as_str() {
            "google" => "Google",
            "github" => "Github",
            _ => {
                let arn = raw.get("profile_arn").and_then(|v| v.as_str()).unwrap_or_default().to_ascii_lowercase();
                if arn.contains("google") {
                    "Google"
                } else if arn.contains("github") {
                    "Github"
    ...
    let mut account = Account::new(String::new(), "kiro-cli".to_string());
    account.provider = Some(provider.to_string());
    account.auth_method = Some(if is_social { "social" } else { "IdC" }.to_string());
    account.access_token = Some(token.access_token.clone());
    account.refresh_token = Some(token.refresh_token.clone());
    account.expires_at.clone_from(&token.expires_at);
    account.region = Some(token.region.clone());
    account.profile_arn = raw.get("profile_arn").and_then(|v| v.as_str()).map(str::to_string);
    if !is_social {
        if let Some(reg) = snapshot.device_registration.as_ref() {
            account.client_id = Some(reg.client_id.clone());
            account.client_secret = Some(reg.client_secret.clone());
            account.region = Some(reg.region.clone());
        }
    }

    let refresh = refresh_token_by_provider(&account).await?;
    crate::kiro::cli::update_cli_token_entry(
        expanded_path,
        &entry.key,
        &entry.value_json,
...
async fn read_cli_db_snapshot_with_auto_refresh(
    expanded_path: &str,
) -> Result<crate::kiro::cli::KiroCliDbSnapshot, String> {
    let snapshot = crate::kiro::cli::read_cli_db_snapshot(expanded_path)?;
    if !cli_snapshot_needs_refresh(&snapshot) {
        return Ok(snapshot);
    }

    match refresh_cli_db_token_inner(expanded_path).await {
        Ok(snapshot) => Ok(snapshot),
        Err(e) => {
            log::warn!("[Kiro CLI] 自动刷新即将过期 token 失败: {e}");
            Ok(snapshot)
        }
    }
}

/// 刷新当前 kiro-cli 数据库中的 token，并写回原 token key
#[tauri::command]
pub async fn refresh_cli_db_token(
    db_path: String,
) -> Result<crate::kiro::cli::KiroCliDbSnapshot, String> {
    let expanded_path = expand_home_dir(&db_path)?;
    refresh_cli_db_token_inner(&expanded_path).await
}

pub async fn refresh_default_cli_db_token_if_expiring() -> Result<bool, String> {
    let db_path = get_kiro_cli_default_path()?;
    if db_path.is_empty() {
        return Ok(false);
    }

    let snapshot = crate::kiro::cli::read_cli_db_snapshot(&db_path)?;
    if !cli_snapshot_needs_refresh(&snapshot) {
        return Ok(false);
    }

    refresh_cli_db_token_inner(&db_path).await?;
    Ok(true)
}
